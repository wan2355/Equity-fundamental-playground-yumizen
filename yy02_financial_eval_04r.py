...
def save_factor_map(df: pd.DataFrame, outpath: Path, color_by: str = "CYCLE") -> None:
    ...
    req = ["Ticker", "F_Valuation_Expensive", "F_Quality"]
    for r in req:
        if r not in d.columns:
            return

    x = pd.to_numeric(d["F_Valuation_Expensive"], errors="coerce")
    y = pd.to_numeric(d["F_Quality"], errors="coerce")

    # size by market cap (robust)
    if "MktCap_B_Latest" in d.columns:
        size_raw = pd.to_numeric(d["MktCap_B_Latest"], errors="coerce")
        s = _scale_0_100(size_raw, higher_is_better=True).fillna(30.0)
        sizes = 80.0 + (s * 6.0)  # 80..680
    else:
        sizes = np.full(len(d), 220.0)

    # color selection
    color_by = (color_by or "CYCLE").upper()
    if color_by == "SW" and "Score_SW" in d.columns:
        cvals = pd.to_numeric(d["Score_SW"], errors="coerce")
        c_label = "Score_SW (points)"
        title = "Factor Map: x=Valuation(Expensive), y=Business Strength, color=SW"
    elif color_by == "HW" and "Score_HW" in d.columns:
        cvals = pd.to_numeric(d["Score_HW"], errors="coerce")
        c_label = "Score_HW (points)"
        title = "Factor Map: x=Valuation(Expensive), y=Business Strength, color=HW"
    elif "Score_CYCLE" in d.columns:
        cvals = pd.to_numeric(d["Score_CYCLE"], errors="coerce")
        c_label = "Score_CYCLE (points)"
        title = "Factor Map: x=Valuation(Expensive), y=Business Strength, color=CYCLE"
    else:
        cvals = pd.to_numeric(d.get("F_Growth", np.nan), errors="coerce")
        c_label = "F_Growth (0-100)"
        title = "Factor Map: x=Valuation(Expensive), y=Business Strength, color=Growth"

    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111)
    sc = ax.scatter(x, y, s=sizes, c=cvals)

    ax.set_xlabel("Valuation (Expensive) 0-100  (higher = more expensive)")
    ax.set_ylabel("Business Strength 0-100  (profitability + resilience)")
    ax.set_title(title, fontsize=10)

    # Labels
    for _, r in d.iterrows():
        xi = pd.to_numeric(r.get("F_Valuation_Expensive", np.nan), errors="coerce")
        yi = pd.to_numeric(r.get("F_Quality", np.nan), errors="coerce")
        if pd.isna(xi) or pd.isna(yi):
            continue
        ax.text(xi + 1.0, yi + 0.6, str(r.get("Ticker","")), va="center")
    ...
    # Bubble size note
    ax.text(0.02, 0.02, "Bubble size ~ Market Cap", transform=ax.transAxes, va="bottom")
